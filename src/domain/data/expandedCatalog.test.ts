...
describe("expanded component catalog", () => {
  it("contains the expanded registry and new compact products", () => {
    expect(Object.keys(componentRegistry)).toHaveLength(49);
  ...
  it("switches to new case without losing a valid empty build", () => {
    let state = blank("case-01");
    for (const componentId of ["case-matx-airflow"]) {
  ...
  it("shares 12 new GLB families across the expanded product definitions", () => {
  ...
  it("routes each storage form factor only to its matching bay", () => {
    const profile = caseProfiles.find((item) => item.componentId === "case-01")!;
    const state = blank(profile.componentId);
  ...
  it("reports mutually exclusive air and liquid CPU cooling", () => {
    const state: BuildState = {
      ...blank("case-matx-airflow"),
      placements: [
        { componentId: "case-matx-airflow", mountId: "case-root" },
