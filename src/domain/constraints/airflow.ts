...
export const validateAirflow = (state: BuildState, context: AirflowContext = {}) => {
  ...
  if (missing.length > 0) {
    return [{ id: "AIRFLOW_DIRECTION_UNCONFIGURED", type: "AIRFLOW" as const, severity: "ERROR" as const, category: "COMPLETENESS" as const, message: `Fan direction is not configured for: ${missing.join(", ")}.`, affectedComponentIds: missing }];
  }
  ...
  if (fans.length > 1 && (intake === 0 || exhaust === 0 || Math.abs(intake - exhaust) > AIRFLOW_MAX_DIRECTION_IMBALANCE)) {
    return [{ id: "AIRFLOW_UNBALANCED", type: "AIRFLOW" as const, severity: "WARNING" as const, category: "COMPLETENESS" as const, message: `Airflow is unbalanced: ${intake} intake / ${exhaust} exhaust.`, affectedComponentIds: fans.map((fan) => fan.componentId) }];
  }
  return [];
};
