  ...
  return (
    <section className="reviewer-panel" aria-labelledby="reviewer-title">
      ...
      <div className="command-row reviewer-actions">
        ...
        <button type="button" onClick={() => run(moveComponentTool({ componentId: "radiator-01", mountId: "radiator-top" }))} disabled={!placements.some((item) => item.componentId === "radiator-01")}>{t("review.move")}</button>
        <button type="button" className="secondary" onClick={() => run(simulateChangesTool({ actions: [{ type: "MOVE_COMPONENT", componentId: "radiator-01", mountId: "radiator-top" }] }))} disabled={!placements.some((item) => item.componentId === "radiator-01")}>{t("review.simulate")}</button>
      </div>
